...
export default function Section({children, id, index, onDelete, onCopy, allStyles}:SectionProps) {
  ...
  return (
    ...
    <div  ref={setNodeRef} style={style} className={` flex-child group relative hover:border hover:border-solid hover:border-blue-600/60   ${isJustDropped && 'border border-dashed  border-blue-600/40 hover:border-solid '}  section-editor ${allStyles?.classes}  `   } >
      

      {children}

    <span ref={setActivatorNodeRef}  
    className={` group-hover:block  absolute top-0 w-full h-full  cursor-grab  ${isJustDropped ? ' ': 'hidden'} `} 
